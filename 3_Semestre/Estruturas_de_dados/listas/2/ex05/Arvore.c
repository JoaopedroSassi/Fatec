...
{
	if(a != NULL) //Verificando se A é null
	{
		if(a->preco < valor) //Verificando se o preco é menor
			printf("Codigo %d | Preco %.2f\n", a->codigo, a->preco); //Printando se for
		
		mostraMenor(a->esq, valor); //Recursando
		mostraMenor(a->dir, valor); //Recursando
	}
}
...
int contaProdutos(Arv *a, float v1, float v2)
{
	...
	if(a != NULL)
	{
		if(a->preco >= v1 && a->preco <= v2)
			contProdutos += + 1;
		
		contProdutos += contaProdutos(a->esq, v1, v2);
		contProdutos += contaProdutos(a->dir, v1, v2);
	}
}
