...
/*int contaProdutos(Arv *a, float v1, float v2)
{
	...
	if(a != NULL)
	{
		...
		contProdutos = contProdutos + contaProdutos(a->esq, v1, v2);
		contProdutos = contProdutos + contaProdutos(a->dir, v1, v2);
	}
}*/
...
{
	if(a->esq != NULL)
		printf("\nFilho a esquerda: %d", a->esq->info);
	else
		printf("\nNao tem filho a esquerda");
	
	if(a->dir != NULL)
		printf("\nFilho a direita: %d", a->dir->info);
	else
